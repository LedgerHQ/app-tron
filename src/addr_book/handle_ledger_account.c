...
static void free_ledger_account_buffers(void) {
    APP_MEM_FREE_AND_NULL((void **) &g_ctx.address_display);
...
bool handle_check_register_ledger_account(ledger_account_t *params) {
    ...
    if (APP_MEM_PERMANENT((void **) &g_ctx.address_display, AB_ADDR_STR_SIZE) == false) {
    ...
    getBase58FromAddress(address, g_ctx.address_display);
...
void display_register_ledger_account_review(nbgl_choiceCallback_t choice_callback) {
    memset(g_ctx.pairs, 0, sizeof(g_ctx.pairs));
    memset(&g_ctx.list, 0, sizeof(g_ctx.list));
    g_ctx.pairs[0].item = "Account name";
    g_ctx.pairs[0].value = g_ctx.ledger_account->account_name;
    g_ctx.pairs[1].item = "Address";
    g_ctx.pairs[1].value = g_ctx.address_display;
    g_ctx.list.pairs = g_ctx.pairs;
    g_ctx.list.nbPairs = 2;

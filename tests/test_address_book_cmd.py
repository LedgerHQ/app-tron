...
from application_client.tron_transaction import contract, pack_contract, tron
...
def check_identity_response(data: bytes, contact_name: str, scope: str, address: bytes):
    ...
    assert hmac_name == compute_hmac_name(gid, contact_name)
    assert hmac_rest == compute_hmac_rest(gid, scope, address)
...
def _approve_ledger_account(nav: NavigateWithScenario, do_compare: bool = True) -> None:
    # Nano: nbgl_useCaseReview. Touch: nbgl_useCaseAdvancedChoiceWithDetails.
    if nav.backend.device.is_nano:
        nav.review_approve(do_comparison=do_compare, custom_screen_text="Confirm")
        return
    ...
    if do_compare:
        nav.navigator.navigate_and_compare(nav.screenshot_path, nav.test_name, instructions)
    else:
        nav.navigator.navigate(instructions)
...
def _register_identity(
    nav, client, contact_name=DEFAULT_CONTACT_NAME, scope=DEFAULT_SCOPE, address=DEFAULT_ADDRESS, do_compare=True
):
...
def _register_ledger_account(nav, client, do_compare=True, derivation_path=DEFAULT_BIP32_PATH, contact_name=DEFAULT_ACCOUNT_NAME):
...
def _transfer_tx(accounts, to_address: bytes, owner_index: int = 0):
    return pack_contract(
        ...
        contract.TransferContract(
            owner_address=bytes.fromhex(accounts[owner_index]["addressHex"]), to_address=to_address, amount=100000000
        ),
    )
...
def test_address_book_identity_edit_contact_name(backend, scenario_navigator):
    ...
    check_hmac_response(
        ...
        compute_hmac_name(group_handle[:GID_SIZE], new_name),
    )
...
def test_address_book_identity_edit_identifier(backend, scenario_navigator):
    ...
    check_hmac_response(
        ...
        compute_hmac_rest(group_handle[:GID_SIZE], DEFAULT_SCOPE, new_address),
    )
...
def test_address_book_identity_edit_scope(backend, scenario_navigator):
    ...
    check_hmac_response(
        ...
        compute_hmac_rest(group_handle[:GID_SIZE], new_scope, DEFAULT_ADDRESS),
    )
...
def test_address_book_ledger_account_edit_invalid_hmac(backend, scenario_navigator):
    ...
    cmd = EditLedgerAccount(
        old_account_name=DEFAULT_ACCOUNT_NAME,
        new_account_name="Trx savings",
        derivation_path=DEFAULT_BIP32_PATH,
        hmac_proof=bytes(b ^ 0xFF for b in hmac_proof_old),
        blockchain_family=FAMILY,
    )
    backend.raise_policy = RaisePolicy.RAISE_NOTHING
